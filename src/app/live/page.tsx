...
export default function Live() {
    ...
    return (
        <div className="container" style={{ maxWidth: '1200px', margin: '0 auto', padding: 'clamp(1rem, 3vw, 2rem)' }}>
            <div className="live-grid" style={{ display: 'grid', gridTemplateColumns: '1fr', gap: '1.5rem' }}>
                {/* Main Live Score */}
                <div>
                    {score && (score.status || score.matchTitle) ? (
                        <div className="card animate-fade-in" style={{ padding: 'clamp(1rem, 3vw, 2rem)', background: 'linear-gradient(145deg, #1e293b, #0f172a)', marginBottom: '1.5rem' }}>
                            {/* Series & Venue */}
                            <div style={{ textAlign: 'center', marginBottom: '1.5rem' }}>
                                <h4 style={{ color: '#94a3b8', fontSize: 'clamp(0.75rem, 2vw, 0.9rem)', textTransform: 'uppercase', letterSpacing: '1.5px', marginBottom: '0.5rem', wordBreak: 'break-word' }}>
                                    {score.seriesName || score.matchTitle}
                                </h4>
                                <p style={{ color: '#64748b', fontSize: 'clamp(0.75rem, 1.8vw, 0.85rem)', margin: 0 }}>
                                    {score.matchLocation}
                                </p>
                            </div>

                            {/* Teams & Scores */}
                            <div className="score-teams" style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: '1rem', gap: 'clamp(0.5rem, 2vw, 2rem)' }}>
                                <div style={{ textAlign: 'left', flex: 1, minWidth: 0 }}>
                                    <h2 style={{ fontSize: 'clamp(1.1rem, 3vw, 1.8rem)', margin: 0, marginBottom: '0.5rem', whiteSpace: 'nowrap', overflow: 'hidden', textOverflow: 'ellipsis' }}>{score.teamA}</h2>
                                    <p style={{ fontSize: 'clamp(1.5rem, 4vw, 2.5rem)', fontWeight: 'bold', margin: 0, color: 'var(--primary)' }}>
                                        {score.scoreA}
                                    </p>
                                </div>

                                <div style={{ display: 'flex', flexDirection: 'column', alignItems: 'center', padding: '0 clamp(0.5rem, 2vw, 2rem)', flexShrink: 0 }}>
                                    <span style={{
                                        background: 'var(--primary)', color: 'white', padding: '0.25rem 0.5rem',
                                        borderRadius: '20px', fontSize: 'clamp(0.65rem, 1.5vw, 0.8rem)', fontWeight: 'bold',
                                        boxShadow: '0 0 10px var(--primary)', marginBottom: '0.5rem',
                                        animation: 'pulse 2s infinite', whiteSpace: 'nowrap'
                                    }}>
                                        LIVE
                                    </span>
                                    <div style={{ fontSize: 'clamp(1rem, 2.5vw, 1.5rem)', color: '#64748b', fontWeight: 'bold' }}>VS</div>
                                </div>

                                <div style={{ textAlign: 'right', flex: 1, minWidth: 0 }}>
                                    <h2 style={{ fontSize: 'clamp(1.1rem, 3vw, 1.8rem)', margin: 0, marginBottom: '0.5rem', whiteSpace: 'nowrap', overflow: 'hidden', textOverflow: 'ellipsis' }}>{score.teamB}</h2>
                                    <p style={{ fontSize: 'clamp(1.5rem, 4vw, 2.5rem)', fontWeight: 'bold', margin: 0 }}>
                                        {score.scoreB}
                                    </p>
                                </div>
                            </div>
